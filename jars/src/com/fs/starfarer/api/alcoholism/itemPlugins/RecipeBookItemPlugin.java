...
public class RecipeBookItemPlugin extends GenericSpecialItemPlugin {
    ...
    public void performRightClickAction() {
        ...
        if(recipeBookSpecialItemData != null){
            for (Alcohol alcohol : recipeBookSpecialItemData.getAlcoholList()){
                cargo.addSpecial(new SpecialItemData(alcohol.getIndustryItemId(), null), 1f);
            }
        }
    }
    ...
    public int getPrice(MarketAPI market, SubmarketAPI submarket) {
        ...
        if(data != null) for (Alcohol alcohol : data.getAlcoholList()) amt += Global.getSettings().getSpecialItemSpec(alcohol.getIndustryItemId()).getBasePrice();
        return amt;
    }
}
